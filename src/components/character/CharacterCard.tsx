      ...
      <div className={view === "list" ? "flex items-start space-x-4" : "w-full h-auto flex flex-col"}>
        <div
          className={`shrink-0 overflow-hidden bg-muted relative ${
            view === "list" ? "w-16 h-16 rounded-lg" : "w-full aspect-2/3 rounded-lg shadow-lg group-hover:shadow-xl"
          }`}
        >
          {imageUrl && !imageError ? (
            <Image
        ...
        <div className={`min-w-0 ${view === "list" ? "flex-1" : "pt-2"}`}>
          <h3 className="font-semibold text-sm group-hover:text-primary transition-colors line-clamp-2">{characterName}</h3>

          {character.name_kanji && (
            <p className="text-xs text-muted-foreground mt-0.5 line-clamp-1">{character.name_kanji}</p>
          )}
          ...
          <div className={`space-y-1 ${view === "list" ? "mt-2" : "mt-1 flex flex-col"}`}>
            ...
            {character.nicknames && character.nicknames.length > 0 && (
              <div className="text-xs text-muted-foreground line-clamp-1 italic">"{character.nicknames[0]}"</div>
            )}
          </div>
        </div>
      </div>
